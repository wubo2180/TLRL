...
class GATConv(MessagePassing):
    def __init__(self, emb_dim, heads=2, negative_slope=0.2, aggr="add", input_layer=False):
        ...
        ### Mapping 0/1 edge features to embedding
        self.edge_encoder = torch.nn.Linear(1, heads * emb_dim)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # print('\n propagate',x.shape,edge_index.shape,edge_embeddings.shape)
        return self.propagate(edge_index, x=x, edge_attr=edge_embeddings)
...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK = "last", drop_ratio = 0, gnn_type = "gin"):
        ...
        self.x_embedding = torch.nn.Linear(node_in_channels, emb_dim)
...
class GNN_graphpred(torch.nn.Module):
    ...
    def from_pretrained(self, model_file):
        ...
        print(f'loading from pre-trained model {model_file}')
        # print(next(torch.load(model_file).parameters().device))
