...
from bio_model import GNN_graphpred
...
def main():
    ...
    parser.add_argument('--model_file', type=str, default = './res/bio/bio_gin_20_gnn.pth', help='filename to read the model (if there is any)')
    ...
    # root_supervised = os.path.join(args.dataset_path, 'dataset/Bio/supervised')
    # root_supervised = dir+'data/bio/supervised'
    root_supervised='./dataset/bio/supervised'
    ...
    if args.split == "random":
        ...
        test_easy_loader = DataLoader(test_dataset_broad, batch_size=args.batch_size, shuffle=False, num_workers = args.num_workers)
        test_hard_loader = DataLoader(test_dataset_none, batch_size=args.batch_size, shuffle=False, num_workers = args.num_workers)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if args.split == "random":
            ...
            # print(test_acc_easy)
            print('test:', test_acc_hard.mean())
            # print(test_acc_hard)
        if test_acc_hard.mean()>best_test_acc:
            best_epoch=epoch
            best_test_acc=test_acc_hard.mean()
        print('best_epoch: {}, best: {}'.format(best_epoch, best_test_acc))

        print("")

    os.makedirs("bio_result/finetune_seed" + str(args.runseed), exist_ok=True)

    if not args.filename == "":
        with open("bio_result/finetune_seed" + str(args.runseed)+ "/" + args.filename, 'wb') as f:
            if args.split == "random":
                ...
                # pickle.dump({"train": np.array(train_acc_list), "val": np.array(val_acc_list), "test": np.array(test_acc_easy_list), "test_hard": np.array(test_acc_hard_list)}, f)
